...
ALL_STATUS_TAGS = ['ray-node-status:uninitialized', 'ray-node-status:waiting-for-ssh', 'ray-node-status:setting-up', 'ray-node-status:syncing-files', 'ray-node-status:up-to-date', 'ray-node-status:update-failed']
RETRIES = 10

WORKER = 'ray-node-type:worker'
HEAD = 'ray-node-type:head'
...
class Gen2NodeProvider(NodeProvider):
    ...
    def log_in_out(func):
      ...
      def decorated_func(*args, **kwargs):
        name = func.__name__
        logger.info(f"Enter {name} from {inspect.stack()[0][3]} {inspect.stack()[1][3]}  {inspect.stack()[2][3]} with args: {args} and kwargs {kwargs}")
        try:
            result = func(*args, **kwargs)
            logger.info(f"Leave {name} from {inspect.stack()[1][3]} with result {result}, entered with args: {args}")
        except:
    ...
    def _init(self):
        with self.lock:
            self.ibm_vpc_client = _create_vpc_client(self.endpoint, IAMAuthenticator(self.iam_api_key))
            self.global_tagging_service = GlobalTaggingV1(IAMAuthenticator(self.iam_api_key))
            self.global_search_service = GlobalSearchV2(IAMAuthenticator(self.iam_api_key))
    ...
    def non_terminated_nodes(self, tag_filters):
        ...
        with self.lock:
            ...
            while retry < NT_RETRIES:
                retry > 0 and logger.info(f'nt_retry #{retry}, self.global_search_service.search with query {query}')
                retry += 1
                result = self.global_search_service.search(query=query, fields=['*'], limit=1000).get_result()
                ...
                for node in items:
                    ...
                    if node['resource_id'] in self.deleted_nodes:
                        logger.info(f"{node['resource_id']} been scheduled for delete")
                        continue

                    # TODO: should be special case here in case of cache_stopped_nodes?
                    if node['doc']['status'] not in ['pending', 'starting', 'running']:
                        logger.info(f"{node['resource_id']} status {node['doc']['status']} not in ['pending', 'starting', 'running'], skipping")
                        continue
                    ...
                    try:
                        ...
                        if e.message == 'Instance not found':
                            logger.error(f"failed to find instance {instance_id}, skipping")
                            continue
                        logger.error(f"failed to find instance {instance_id}, raising")
                        raise e
...
#                    import pdb;pdb.set_trace()
                    ...
                    if missing_nodes and sorted(prev_nodes_keys - found_nodes.keys()) != missing_nodes:
                        logger.info(f'missing nodes detected in previous retry don\'t match currently missing nodes')
                        # there a chance for infinite loop here?
                        retry = 0
                    
                    missing_nodes = sorted(prev_nodes_keys - found_nodes.keys())

                    logger.warning(f'{prev_nodes_keys - found_nodes.keys()} from perviously cached node missing in the list of found_nodes {found_nodes.keys()} found using query {query}, retrying search, retries left: {NT_RETRIES - retry}')
                    time.sleep(3)
                    continue
                else:
                    ...
                    for node in nodes:
                        if not any("ray-node-status" in t for t in node['tags']):
                            logger.warning(f'{node["resource_id"]} is missing ray-node-status in tags: {node["tags"]}, retrying')
                            bad_tags = True
                            break
...
#            logger.info(f'in _tags_to_dict - {_tags_split}')
            ...
            try:
                if f_name == 'attach_tag':
                    ...
                    if not tag_results['results'][0]['is_error']:
                        ...
                        node_tags = [t['name'] for t in self._global_tagging_with_retries(resource_crn, 'list_tags')]

                        logger.info(f'instance current tags {node_tags}')
                        if set(tags) - set(node_tags):
                            logger.info(f'instance {resource_crn} missing attached tags {set(tags) - set(node_tags)}, retrying')
                            continue
                        else:
                            return tag_results
                    ...
                    if not tag_results['results'][0]['is_error']:
                        ...
                        if set(node_tags) & set(tags):
                            logger.info(f'instance {resource_crn} still having dettached tags {set(node_tags) & set(tags)}, retrying')
                        else:
                            return tag_results
    ...
    def set_node_tags(self, node_id, tags):
        ...
        with self.lock:
            ...
            if not self._global_tagging_with_retries(resource_crn, 'attach_tag', tags=new_tags):
                cli_logger.error(f"failed to tag node {node_id} with tags {new_tags}, raising error")
                raise
    ...
    def _create_node(self, base_config, tags):
        ...
        if not self._global_tagging_with_retries(instance['crn'], 'attach_tag', tags=_tags):
            # terminate the weird node that failed to be tagged
            logger.error(f"failed to tag node {instance} with tags {_tags}, terminating instance")
            self._delete_node(instance['id'])
            return {}
    ...
    def _get_node(self, node_id):
        ...
        with self.lock:
            ...
            try:
                ...
                # TODO: what to do with a weird node that failed to list tags for?

            except Exception as e:
                logger.error(f'Error listing tags for node {node}: {e}')
                raise e

            return {"resource_id": node['id'], 'crn':  node['crn'], 'doc': node, 'tags': [t['name'] for t in node_tags]}
